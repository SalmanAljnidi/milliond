...
// --- دالة السؤال الجديدة (منطق القسمة) ---
function newQuestion() {
    ...
    // منطق القسمة بحيث يكون الناتج والمقسوم عليه من 1 إلى 10
    const divisor = Math.floor(Math.random() * 10) + 1; // المقسوم عليه (1-10)
    const result = Math.floor(Math.random() * 10) + 1;  // الناتج المطلوب (1-10)
    const dividend = divisor * result;                  // المقسوم (مثلاً 5 * 6 = 30)

    correctVal = result; // الجواب الصحيح هو الناتج
    ...
    for(let i=0; i<4; i++) {
        if(i !== correctIndex) {
            let w;
            do {
                // الخيارات الخاطئة يجب أن تكون في حدود المعقول (1-10) أو قريبة
                // بما أن الناتج عدد واحد (1-10)، نجعل الخيارات الخاطئة أيضاً في هذا النطاق
                w = Math.floor(Math.random() * 10) + 1; 
                // إذا تكرر الرقم أو كان نفس الناتج الصحيح، نختار رقم عشوائي آخر (قد يكون أكبر قليلاً لإرباك المتسابق)
                if (w === correctVal || answers.includes(w)) {
        ...
        if(idx === correctIndex) {
            ...
            setTimeout(() => {
                ...
                if(currentLevel >= prizes.length) {
                    showModal('🎉 مبروك المليون 🎉', `ألف مبروك يا ${playerName}! أنت عبقري في القسمة وحصلت على المليون!`);
...
function usePhone() {
    ...
    setTimeout(() => { document.getElementById('modal-content').innerHTML = `<div style="font-size:40px">🤠</div><p>هلا والله يا <b>${playerName}</b>!<br>القسمة سهلة.. الجواب هو <b>${toArabic(correctVal)}</b></p>`; }, 3000);
}
